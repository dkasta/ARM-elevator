...
int main (void) {
  ...
  button_init();													/* BUTTON Initialization              */
	...
	init_timer(TIMER2, freqs[ indici[0] ], 3, 0); //produce una specifica nota presa da vettore freqs
	
	/* 
	Valori dei periodi dei timer in esadecimale
	0x0ABA9500 = 7.2s * 25MHz 
	0x59682F00 = 60s * 25MHz
	0x005F5E10 = (1/(2*2)) * 25MHz (blink a 2 Hz durante trasporto)
	0x002625A0 = (1/(5*2)) * 25MHz (blink a 5 Hz all'arrivo al piano)
	0x002FAF08 = (1/(4*2)) * 25MHz (blink a 4 Hz durante modalità emergenza)
	*/
